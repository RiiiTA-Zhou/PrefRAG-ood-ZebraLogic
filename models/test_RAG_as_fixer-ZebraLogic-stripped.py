'''
Test the #Solve/#Question stripping variant of PrefRAG on ZebraLogic.

This is a modified copy of test_RAG_as_fixer-ZebraLogic.py that uses
StrippedRAGFixer instead of RAGFixer. The key differences:
  1. Before retrieval: #Solve is stripped from the query program so the
     embedding focuses on declarations + constraints.
  2. After retrieval: #Question sections are stripped from retrieved
     corpus entries so the LLM sees only the logic parts in the examples.
  3. The question prompt still contains the full original program (with #Solve),
     so the LLM can correct it end-to-end.

Only processes the first N samples (default 5) for quick experiments.
Uses a separate output file to avoid interfering with existing results.
'''

from utils.utils_stripped import StrippedRAGFixer, strip_solver_section
from utils.utils import OpenAIModel, execute_logic_program, loading_kb_with_da
...
# ------------ SETTING ------------
parser = argparse.ArgumentParser(description="Test stripped-query RAG on ZebraLogic")
...
# Different output file to avoid conflict with original results
result_jsonl_path = os.path.join(result_save_dir, f"result-{llm_name}-stripped.jsonl")
...
# Use the STRIPPED RAGFixer
Fixer = StrippedRAGFixer(dataset_name, retrievers)
# ------------ RAG setup ------------
    ...
    for judge_num in range(3):
        ...
        for attempt in range(3):
            try:
                ...
                import traceback
                print(f"  Fixer attempt {attempt} failed: {e}")
                traceback.print_exc()
                time.sleep(1)
...
print(f"Output saved to: {result_jsonl_path}")
